...
//	@Parameter(default_value = "", description = "groundwater storage coefficient,varied by soil types", range = "", unit = "")
        ...
        if (dgl >= Ds) {
            ps1 = SuctionFromMoisture_V(w[layer - 1], wsat, wrsd, watern, alpha);
            tmp = wfld * (1.0 - (dgl - Ds) / Dg);
            tmp = Math.max(tmp, wrsd);
            ps2 = SuctionFromMoisture_V(tmp, wsat, wrsd, watern, alpha);
            avz[layer - 1] = 0.5 * (D[layer - 1] + (dgl - Ds));
            avk[layer - 1] = conductivity_V(k0[layer - 1], wsat, wrsd, watern,
                    w[layer - 1]);
            ...
            if (rech >= 0.0) {
                ...
                tmp = Math.max(0.0, (wsat - w[layer - 1]) * D[layer - 1]);
