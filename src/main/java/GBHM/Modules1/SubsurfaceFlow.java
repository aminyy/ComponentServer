        ...
        if (dgl - Ds < 0 && Drw >= Dr && dgl <= 0.0) {
            ...
            if (qsub < 0.0) {
                ...
                if (GWst <= Dg * GWcs) {
                    ...
                    if (dgl < 0.0) {
                        System.out
                                .println(this.getClass().getName() + " wrong in renewing groundwater table: dg1 = "
                                + dgl);
                    }
                } else {
                    tmp = GWst - Dg * GWcs;
                    ...
                    if (Math.abs(w[layer - 1] - wsat[layer - 1]) < 0.1E-5) {
                        dgl = Ds - D[layer - 1];
                        GWkey = 1;
                    } else {
                        GWkey = 0;
                    ...
                    //added by Long Yinping 2013.12.14
                    if (excess > 0) {
                        qsub = qsub + excess;
                    }
                    //end of adding
                ...
                if (dgl >= Ds - 0.01) {
                    ...
                    /*
                     ...
                     if (i == -1) {
                     if (Math.abs(w[i + 1] - wsat[i + 1]) <= 0.1E-3) {
                     isat = i + 1;
                     }
                     } else {
                     if (Math.abs(w[i] - wsat[i]) <= 0.1E-3) {
                     isat = i;
                     }
                     }*/
                    //changed by Long Yinping 2013.12.14
                    for (i = layer - 1; i >= 0; i--) {
                        tmp_D = tmp_D + D[i];
                        if (Math.abs(w[i] - wsat[i]) <= 0.1E-3) {
                            isat = i;
